...
namespace ClassicAssist.UO.Data
{
    public static class Art
    {
        ...
        private static Dictionary<int, Entry3D> LoadMULIndex()
        {
            ...
            using (FileStream reader = File.Open(Path.Combine(_dataPath, "artidx.mul"), FileMode.Open, FileAccess.Read, FileShare.ReadWrite))
            {
                int bytesRead;
                do
                {
                    bytesRead = reader.Read(buffer, 0, entrySize);
                    indexes[index++] = (Entry3D)Marshal.PtrToStructure(pinnedBuffer.AddrOfPinnedObject(), typeof(Entry3D));
                } while (bytesRead > 0);
            }
